...
/* globals buildPositionList, buildDepartmentList, lists, quill */
...
function buildAccessList(list) {
  ...
  output += "<div class='UI-table-cell'><div class='UI-table-vertical'>&nbsp;</div></div>";
  output += "</div>\n";
  output += "</div>\n";
  var dept = Array();
  var position = Array();
  ...
  list.forEach(function(element, index) {
    ...
    output += "<div class=UI-table-cell><button onclick='removeAccess("+idx+")' class='UI-roundbutton'><i class='fa fa-minus-square'></i></button></div>";
    output += "</div>\n";
    idx += 1;
  });
  // Blank One

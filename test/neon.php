...
class Neon 
{
  ...
  public function login($keys) {
    if (isset($keys['orgId']) && isset($keys['apiKey'])) {
      ...
      if ($response['operationResult'] == 'SUCCESS') {
        ...
        die("Failed to login to the NeonCRM System with OrgID '" . $keys['orgId'] . "'.  Check the login/API settings in .ht_neon.php!  Error returned: " . $response['errors']['error'][0]['errorMessage']);
      } 
    } 
    else {
      die("Failed to login to the NeonCRM System - missing OrgId.  Check the login/API settings in .ht_neon.php!");
    }
  }
  ...
  public function search($request) {
    ...
    if (isset($request['method'])) {
      ...
      if (isset($request['parameters'])) {
        $param = '&' . http_build_query($request['parameters']);
        }
      ...
      $parameters = $addon . $param . $criteria . $standard . $custom . $paging;
      $build = array();
      $build['method'] = $request['method'];
      $build['parameters'] = $parameters;
      $go = $this->api($build);
